---
title: "Analyzing MetaPopGen Results (analysis script V2)"
...
incorporate real data into analysis, for comparison with simulated data.


Real data, recently moved to:

Copy_of_real_data_PhfoAllGenePop_13Dec2016.gen

Q:\Research\All_Projects_by_Species\Phacelia SPECIES\Phacelia_formosula\Phacelia formosula_abiotic\Modelling\MetaPopGen\


Simulation for practice calculations (for now):
...
run script for these pieces contained in:
MetaPopGen_testing_V10_3_15_17

testing_V8 actually used to run, testing_V9 and testing_V10 contain same run script but add analysis tools. Parameter set for this test run called "V8" throughout these files.
...
Get the file, reading with adegenet tools that take a genepop.gen file and make a genind object
```{r}
...
#Note one odd error: "Duplicate individual names detected. Coercing them to be unique.""
```
...
print(Real_data.genind$strata)
```

Great! Now try calculations. 

First off, need to map these site codes to the three putative species/site map - copy of a file that maps sites to speces here:
...
```
*************************************************************************************
#Run some analyses on real data

```{r}
library(strataG)
print(real_data_by_species.genind)
print(real_data_by_species.genind$all.names)
...
#Instead of loading previously calculated analysis results, run with more permutations

#Code for loading old processed data files. 
...
**************************************************************************************
**By "running_V2" scripts, this is done before saving data. for earlier stuff (including this, still need to subsample here)
...
filepath <- "Q:/Research/All_Projects_by_Species/Phacelia SPECIES/Phacelia_formosula/Phacelia formosula_abiotic/Modelling/MetaPopGen/test_V8_analysis_3_22_17_51t_1000perm_popstat_results.RData" 
