...
```{r}
#alternate
numalleles <- 8  # allow there to be 8 possible states of the microsatellite at this locus [2] number of alleles at this locus
frequencylist <- c(rep(1/numalleles,numalleles)) 
```
...
landscape.amova(rland) #Phi ST for every locus in teh landscape
...
alleles <- seq(7,length(rland.df)-1,2) # to use in lapply, 1:6 are descriptive, 7 on are diploid loci
# genind format wants each locus as alleles separated by some character, "-"
rland.2genind <- sapply(alleles, function(x){
  paste(rland.df[,x], rland.df[,x+1], sep = "-")
})

# Populaitons are marked with dummy variables 0 through (2*num of pops)-1  
# starts at 0, lists stages for each subpop

#GARRRRR, not quite
rland.df$pop <- sapply(seq(0,(habitats*stages)-1,stages),function(c){
  if(rland.df$V1 %in% seq(c,c+stages,1)){
    paste("pop",c,sep="")
  }
}) 
